...
このリファレンスは、変更内容に応じて実行するテストと、再現可能な検証手順の正本です。  
画面の見た目は[デザイン](../specifications/design.md)、利用者から見える動作は[プロダクト仕様](../specifications/product.md)、内部構造は[アーキテクチャ](../specifications/architecture.md)、安全性保証は[セキュリティ](../specifications/security.md)を参照してください。  

## 基本方針

テストは、開発者やAIエージェントによる変更で、意図しない破壊や仕様からの逸脱を防ぐガードレールです。  
まず変更箇所に近いテストを実行し、完了前にその変更を含む範囲まで検証を広げます。  
ドキュメントだけを変更した場合は`mise run lint:docs`、コードや設定も変更した場合は`mise run lint`を実行します。  
フレーキーなテストには、分かっている再現条件と修正方針をコードコメントで残します。  

GitHub Actionsの実行コストを抑えるため、テスト用のCIワークフローは追加しません。  
代わりにLefthookのプッシュ前フックで`mise run ci --smoke`を実行し、静的検査、型検査、セキュリティ検査、単体テスト、統合テスト、主要なE2Eテストとビジュアルリグレッションテストをローカルで確認します。  
変更を提出する前は`mise run ci`で全てのテストを確認します。  
通常アプリは`mise run build`またはリリース時に構築します。  
配布物固有の検証は[リリース](release.md)に従います。  
...
| 主要経路のテスト | `mise run test --smoke` | 単体テスト、統合テスト、主要なE2Eテストとビジュアルリグレッションテスト |
| 全ての検査 | `mise run ci` | 静的検査、型検査、セキュリティ検査、全てのテスト |
| プッシュ前の検査 | `mise run ci --smoke` | 静的検査、型検査、セキュリティ検査、主要経路のテスト |
| READMEの画面画像 | `mise run screenshot` | `screenshots`に置くREADME用画像 |

## 検証を重複させない

受け入れ条件ごとに証拠を確認する担当を一つに決めます。  
手動、ネイティブ、視覚確認はtesterが担当し、必須確認を実施できなければ`PASS`ではなく`BLOCKED`とします。  
同じ差分で成功した同一コマンドの結果は後続の担当が再利用し、未検証の層だけを追加します。  
ソースまたは差分が変わった場合は、影響する検査を再実行します。  

複数のE2E仕様を確認する場合は、一回の`mise run test:e2e`へまとめ、同じ変更状態でネイティブ実行ファイルを繰り返し構築しません。  
`mise run test`と`mise run ci`は`target/release/Stella (E2E)`を一度だけ構築し、E2Eとビジュアルリグレッションで共有します。  
通し実行でも、ビジュアルリグレッションのポート範囲とデータ領域はVRT用に分離します。  
`--smoke`を指定すると、ネイティブアプリの起動、リポジトリの追加と初期化、差分からコミットまでの操作、および差分画面の画像比較だけを実行します。  

```sh
mise run test:e2e -- \
  --spec app/test/e2e/history.spec.ts \
  --spec app/test/e2e/diff.spec.ts
```

検証手段が非対応またはno-opと一度確認できた場合は、同じ方式を再試行しません。  
異なる代替手段を一つだけ試し、解消しなければ制約または`BLOCKED`として報告します。  
必須の証拠と最終判定が揃った後は、追加のビルドや別方式の再確認を行いません。  

## セキュリティに影響する変更を検査する

信頼境界、権限、入力、パス、外部コマンド、機密情報、依存関係、更新、公開へ影響する変更では`mise run security`を実行します。  
依存関係の変更、プッシュ前、リリース前にも同じ検査を実行します。  
個別検査の構成と外部通信は`mise.toml`、例外と失敗判定は[セキュリティ](../specifications/security.md#失敗と警告の判定)に従います。  
...
E2E、ビジュアルリグレッション、スクリーンショット撮影を単独で実行した場合は、次の実行ファイル、バンドルID、組み込みWebDriverポートの探索範囲を使い、Viteの開発サーバーを起動しません。  
...
Tauriの組み込みWebDriverでは、W3C wheel入力が成功応答を返しても画面へ反映されない場合があります。  
この入力結果をスクロールを含む製品動作の証拠には使いません。  
実操作の確認が必要な場合は、表示したネイティブ画面で手動確認します。  
