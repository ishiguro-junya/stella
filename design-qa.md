...
- 参照画像: `/Users/ishiguro/Desktop/スクリーンショット 2026-08-15 6.19.22.png`
- 実装画像:
  - `/Users/ishiguro/project/stella/.tmp/tooltip-qa-final/tooltip-en-light-1180x760.png`
  - `/Users/ishiguro/project/stella/.tmp/tooltip-qa-final/tooltip-en-dark-1180x760.png`
- 比較画像: `/Users/ishiguro/project/stella/.tmp/tooltip-qa-final/tooltip-comparison-final-20260815.png`
- 表示領域: `1180 x 760 CSS px`
- 状態: 変更画面のフェッチをキーボードフォーカス、ライト／ダーク

### ツールチップの全体比較

- 参照画像とダークモードの実装画像を同じ比較画像で開き、明るい吹き出し、下向きの矢印、角丸、影を確認しました。
- ツールチップはフェッチボタンの中央上に表示され、矢印がトリガーを指しています。
- ライトモードでは濃色、ダークモードでは明色へ反転し、どちらも画面背景とのコントラストを確保しています。

### ツールチップの拡大比較

- トリガーとツールチップの中心差は`0px`、間隔は`8px`、表示方向は`top`です。
- ライトモードは背景`rgb(29, 30, 34)`、前景`rgb(255, 255, 255)`です。
- ダークモードは背景`rgb(248, 248, 250)`、前景`rgb(28, 28, 30)`です。
- 矢印の背景色が吹き出し本体と一致することを算出済みスタイルで確認しました。
- 最大幅280px、8px角丸、上下8px・左右12pxの余白、共通の影を維持しています。

### ツールチップの比較履歴

1. 初期実装
   - 参照画像に合わせ、共通コンポーネント、ポータル、矢印、角丸、影、ライト／ダーク配色を実装しました。
2. 位置の問題
   - 重要度: P2
   - 指摘: ネイティブWebViewでウィンドウ寸法を変更したあと、ポータル上のツールチップが変更前の座標に残りました。
3. 位置修正
   - 修正: トリガーと文書ルートを`ResizeObserver`で監視し、レイアウト変更時に共通コンポーネント内で位置を再計算するようにしました。
   - 修正後の確認: `860 x 560`で表示後に`1180 x 760`へ変更しても、中心差`0px`、間隔`8px`を維持しました。
4. テーマ調整
   - 修正: ライトモードでは濃色、ダークモードでは明色の高コントラスト配色へ統一しました。
   - 修正後の確認: 両表示モードの本体、文字、矢印色をネイティブE2Eとスクリーンショットで確認しました。

### ツールチップの指摘事項

対応が必要なP0、P1、P2は残っていません。  

### ツールチップの操作・回帰確認

- 単体テスト499件で、ホバー遅延、フォーカス、非表示操作、上下反転、画面端補正、`aria-describedby`の保持、ネイティブ`title`の除去を含む各機能を確認しました。
- ネイティブE2Eは7ファイル42件すべて通過し、ポインター／キーボード、ライト／ダーク、画面端、ダイアログを閉じる操作、単一・複数ファイル差分を開閉する操作、三点メニューを確認しました。
- `mise run typecheck`、`mise run lint:frontend`、`mise run test:frontend`、`mise run test:e2e`はすべて通過しました。
- アプリ由来のコンソールエラーはありませんでした。
