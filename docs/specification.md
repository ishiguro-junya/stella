...
上部navigationの変更、履歴、活動、設定は、選択中もグレーの背景と通常の文字色を使用します。  
...
変更の左ペインに表示するファイル一覧は、設定から「ファイル名とパス」、「フルパス」、「ツリー」を選択できます。  
初期値の「ファイル名とパス」は、ファイル名と親ディレクトリのパスを2段で表示します。  
「フルパス」はリポジトリからの相対パスを1段で表示します。  
...
一覧領域は5項目分の高さを常に確保し、項目の読み込み前後で画面の高さを変えず、6項目以上は領域内をスクロールします。  
