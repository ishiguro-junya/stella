...
更新がある場合は、起動時の確認画面に加えて、タイトルバーの「変更差分」の左へ青い「更新」ボタンを表示します。  
...
Git操作またはアクティビティの実行中は更新を開始できません。  
...
言語、外観、自動アップデート、ステージ表示、Conventional Commits、Gitツールチェインは、すべてselectで選択します。  
