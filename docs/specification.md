...
Gitがバイナリとして扱う画像とSVGは、各file headerの画像アイコンからプレビューを表示できます。  
内容差分のないrenameは、WebKitで画像としてデコードできた場合だけ画像アイコンを表示します。  
...
画像プレビューボタンはDiff headerで鍵と鉛筆の右、file menuの左に表示し、Editor headerには表示しません。  
...
画像を含むCommitでは右ペインのCommit操作メニュー左に画像プレビューボタンを表示し、画像欄だけを表示または非表示にします。  
テキストとバイナリが混在するCommitでは、画像プレビュー中も既存のテキスト差分を常に表示します。  
