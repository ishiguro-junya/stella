...
- Settingsは左側の幅200pxの一覧から一般、権限、外観、変更、エディタ、Gitを切り替え、右側の詳細paneだけをスクロールします。  
  選択式の設定項目はselectで選択し、リポジトリの保存先、グローバル無視リスト、Gitツールチェーンの操作欄は説明の下に表示します。
  グローバル無視リストはGitカテゴリの先頭に複数行入力で表示します。
  リポジトリの保存先とグローバル無視リストは最大560px、Gitツールチェーンの選択欄は220pxで表示します。
...
- 登録済み一覧とリポジトリ切替は、同じ原因表示と復旧入口を使用します。
- ローカルの問題は「場所を確認」、リモート不在は「リモートを確認」、認証は「認証を確認」、接続は「接続を確認」と短く区別します。
- 場所の確認画面は以前と新しい絶対パスを縦に並べ、自動判定しないことを説明した後に付け替えます。
- 未保存内容を持つ場合は作業画面の上へ確認画面を重ね、場所の選び直しと破棄して閉じる操作だけを提供します。
- リモートURL画面はリモートごとにフェッチURLとプッシュURLを分け、既存URL単位の変更と明示的なフェッチだけを提供します。
- URL変更は入力直後に実行せず、既存の影響確認画面を挟みます。
  確認画面を閉じた後はURL入力へフォーカスが戻ります。
- 一覧と各確認画面は日本語・英語の部品テストで、キーボード操作とフォーカス復元を確認します。
  リモートURL修正後の再フェッチはネイティブE2Eで確認します。
